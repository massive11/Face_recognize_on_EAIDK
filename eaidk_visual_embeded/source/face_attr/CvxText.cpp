...
#include "header/face_attr/CvxText.hpp"
 
// 打开字库
//CvxText::CvxText(const char* freeType)
CvxText::CvxText(char const* freeType)
{
		...
		for(int j = 0; j < cols; ++j) {
			...
			if (slot->bitmap.buffer[off] & (0xC0 >> (j%8))) {
				...
				if(r >= 0 && r < img.rows && c >= 0 && c < img.cols) {
					cv::Vec3b pixel = img.at<cv::Vec3b>(Point(c, r));
					Scalar scalar = Scalar(pixel.val[0], pixel.val[1], pixel.val[2]);
