...
public:
  ...
  */
  int putText(Mat& img, char* text, cv::Point pos);
  ...
  * \return 返回成功输出的字符长度，失败返回-1.
  */
  int putText(Mat& img, const wchar_t* text, cv::Point pos);
  ...
  * \return 返回成功输出的字符长度，失败返回-1.
  */
  int putText(Mat& img, const char* text, cv::Point pos, cv::Scalar color);
  ...
  * \return 返回成功输出的字符长度，失败返回-1.
  */
  int putText(Mat& img, const wchar_t* text, cv::Point pos, cv::Scalar color);
 
private:
  // 禁止copy
  CvxText& operator=(const CvxText&);
  // 输出当前字符, 更新m_pos位置
  void putWChar(Mat& img, wchar_t wc, cv::Point& pos, cv::Scalar color);
